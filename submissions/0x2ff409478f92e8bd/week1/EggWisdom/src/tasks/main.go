...
func main() {
	...
	o.Tx("admin/create_phrase",
		WithSigner("account"),
		WithArg("phrase", "NO GAS WARS HERE, ONLY EGG WARS"),
	...
	o.Tx("admin/create_phrase",
		WithSigner("account"),
		WithArg("phrase", "All The Wisdom"),
		WithArg("base64Img", "The wisdom egg life is short, but the wisdom is eternal"),
		WithArg("namesOnScreen", `["Axlocity", "KittyCatRightMeow", "Noahoverflow"]`),
		WithArg("catsOnScreen", `["Pride", "Prince", "Page"]`),
		WithArg("background", "Dawn"),
	).Print()
	// Fetch phrases``
	o.Script("get_all_phrases").Print()
